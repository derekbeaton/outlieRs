...
multiOut <- function(
  ...
  return(list(outlier_decision=basicOut,
              hidden_detail=list(initDat=as.data.frame(dat),RMDO=outs1, RMDOalpha=rmdo_alpha, Contribs=contVar1, corrMat=rmdos[["corrMat"]], CutOff=outCut, mcdUsed=rmdos$mcdUsed)))
